...
2. **List the different categories of volcanic rocks and explain the differences between the magmas they came from.**
  ...
  3. **Pyroclastic Material** - Pyroclastic (Greek for "fire piece", or hot fragment) material refers to fragments of all sizes that are erupted explosively out of a volcano. Most pyroclastic material or **tephra** form from high-silica magma, although mafic and intermediate composition pyroclasts are not unheard of.  
    1. These range from the finest material, **ash** (< 2 mm diameter) through **lapilli** (2 - 64 mm) to larger **blocks** and **bombs** (> 64 mm). Many pieces of pumice are actually pumice lapilli.
    2. Mafic pyroclasts are called **scoria**.
...
17. **List the three types of plate boundaries and the different types of volcanoes that occur at these plate boundaries.**
  ...
  2. **Convergent margins**: two places collide, one plate subduct into the mantle beneath the other mantle. This holds true for ocean-ocean collisions and ocean-continent collisions, and **volcanic arcs** form in the overlying plate in these settings. Where two continental plates collide, however, neither will subduct, as both are too buoyant to descend into the dense mantle. Rather, they suture together in a massive collisional zone to form a large mountain chain. **Magmatism is rare** in continent-continent collision zones.
    1. ocean-ocean convergent margins: volcanoes in oceanic arcs are composite volcanoes ( magma produced in oceanic arcs are mostly mafic, with some intermediate magmas.)
    2. ocean-continent convergent margins: composite volcanoes (Since continents have more SiO2, volcanoes that result would produce intermediate and felsic magma)
